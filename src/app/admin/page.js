...
export default function AdminDashboard() {
  ...
  if (user?.role !== 'admin') return <div className="text-center py-20 text-[#7777aa]"><div className="text-4xl mb-4">🚫</div><h3 className="text-lg font-bold text-white mb-2">Access Denied</h3></div>;
  ...
  return (
    <div className="max-w-6xl mx-auto px-5 py-8">
      ...
      {msg && <div className="p-3 mb-4 rounded-lg text-sm font-semibold bg-[rgba(46,204,113,0.08)] border border-[rgba(46,204,113,0.15)] text-[#2ecc71]">{msg}<button onClick={() => setMsg('')} className="float-right text-[#7777aa]">✕</button></div>}
          ...
          <div className="bg-[#111122] border border-[rgba(255,255,255,0.06)] rounded-xl p-6">
            ...
            {tournaments.slice(0, 4).map((t) => (
              <div key={t.id} className="flex justify-between items-center py-2 border-b border-[rgba(255,255,255,0.06)] last:border-0">
                ...
                <span className="text-xs text-[#7777aa]">{(t.slotsFilled ?? 0)}/{t.totalSlots ?? 10}</span>
              </div>
            ))}
          </div>
          <div className="bg-[#111122] border border-[rgba(255,255,255,0.06)] rounded-xl p-6">
      ...
      {tab === 'users' && (
        <div>
          ...
          <div className="bg-[#111122] border border-[rgba(255,255,255,0.06)] rounded-xl overflow-hidden">
            <table className="w-full">
              <thead><tr>{['ID','Username','Email','Tokens','Role'].map(h => <th key={h} className="text-left p-3 text-xs font-bold text-[#7777aa] uppercase border-b border-[rgba(255,255,255,0.06)] bg-[rgba(255,255,255,0.02)]">{h}</th>)}</tr></thead>
              <tbody>{users.map(u => <tr key={u.id} className="border-b border-[rgba(255,255,255,0.06)]"><td className="p-3 text-xs text-[#7777aa]">{u.id}</td><td className="p-3 text-sm font-semibold">{u.username}</td><td className="p-3 text-sm">{u.email}</td><td className="p-3 text-sm">{u.tokens ?? 0}</td><td className="p-3 text-sm capitalize">{u.role || 'player'}</td></tr>)}</tbody>
            </table>
          </div>
        </div>
      )}
